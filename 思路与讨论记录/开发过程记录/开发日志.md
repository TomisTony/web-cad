...
所以决定不做装配体了，转向直接把所有 part 组成的真正的 part tree。灵感来源为[Chapter 4 传统3D实体建模（Part工作台） | 请 Inventor 用 FreeCAD (bookdown.org)](https://bookdown.org/johnqu1982/FFI/part.html)，最后也打算大概做成这个样子。
...
今天修复了 Download 的时候会碰到 401 的问题。之后要记得 401 的时候跳转 login 界面，不过这也是后来的事情了。
...
那么我们有没有可能转变思路，直接不管这个 BUG 了，或者这么说，这个本身就不是 BUG，本身就是应该这样的。于是我们的逻辑应该是这样：
...
修正了一个 converte 的时候 edge 的遗存问题（但是这说明了我们有直接新建一个没有 face 的 edge 的能力）
...
{
    "structure": {
        "label": "solid 1", // 固件的显示名称
        "id": "...", // 全局唯一 UUID
        "faces": [], // 固件所拥有的 face 的 id 数组
        "edges": [], // 固件所拥有的 edge 的 id 数组
        "children": [] // 固件所拥有的子节点数组，每个子节点都是相同格式的 "structure"
    },
    "faces": [
        {
            "id": "...", // 根据几何特征生成的 MD5 字符串
            "vertexCoordinates": [], // 顶点坐标数组
            "uvCoordinates": [], // uv 坐标数组
            "normalCoordinates": [], // 法线坐标数组
            "triangleIndexes": [], // 三角面片对应的顶点序号数组
            "numberOfTriangles": 20, // 三角面片总数
    	},
    ],
    "edges": [
        {
...
```json
{
    "structure": {
        "label": "solid 1", // 固件的显示名称
        "id": "...", // 全局唯一 UUID
        "status": "changed", // status 有 "unchanged"/"children_changed"/"changed"
        "faces": [], // 固件所拥有的 face 的 id 数组
        "edges": [], // 固件所拥有的 edge 的 id 数组
        "children": [] // 固件所拥有的子节点数组，每个子节点都是相同格式的 "structure"
    },
    "delete": {
      	"face_ids": [], // 被删除的源 BrCAD 的face的 id 数组
        "edge_ids": [], // 被删除的源 BrCAD 的edge的 id 数组
    },
    "add": {
      	 "faces": [
            {
...
修复了 SidePanel 前端显示为时间戳的问题



## 24-4-11

写中期报告的时候发现自己对于 id 设计的原则有些遗忘。

id 设计主要需要考虑唯一性和不变性。

唯一性指的就是相同几何特征生成的id全局唯一。也表示 solid id 全局唯一。

不变性指的是经过序列化和反序列化 TopoDS_Shape 和 TopoDS_Shape 生成 BrCAD 后，id 依旧不会变化。而一旦几何特征发生了变化，id 就会发生变化
...
NMD 给后端加一个锁怎么就这么难。本来是想让请求发现另一个请求已经给记录上锁了就直接返回，可是到最后还是没法实现这个事情。因为似乎无法直接上禁止读的锁。

现在做并发控制就是在添加进 project 的 operation_history_ids 的时候检查 last_operation_id 还是不是请求发过来的 last_operation_id。然后给这个过程加上事务原子性，确保这个过程中的 project 上了互斥锁。
...
- 把第四节修改一下
  - 补一些细节，特别是 HistoryList 那边，作为把多人协同放到第五节的补偿
  - 补一些前端的实现，特别是需要补图，让别人知道做的大概长什么样
- 把 5.1 和 5.2 补一下
...
添加了 IGES 适配，修了一些 BUG



## 24-5-6

修复 update 项目信息的错误成功提示

添加 .igs 支持；修复了摄像机在特定情况下移动缓慢的问题（其实就是改了个enableDamping = false，拖动手感变差了，但是不会存在越拖越慢的情况了）



## 24-5-7

今天测试的时候突发发现 Boolean 报错了

![image-20240507152255571](assets/image-20240507152255571.png)

![image-20240507152328429](assets/image-20240507152328429.png)

后来想了想，应该是因为 old 里面有两条边，但是经过 boolean 了之后，new 里面只有一条边了。但是这边判断的时候是用 list 不带 copy 判断的，所以第一条边被删除了之后，第二条边依然能过 if，但是 list_copy 里面已经没有这条边了。所以解决方案就是在 if 的时候使用 copy。
