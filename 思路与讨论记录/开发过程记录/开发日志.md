...
突然发现一个严重的问题。我们在对操作好的 TopoDS_Shape 进行 converte 的时候，我们不知道某个 TopoDS_Edge 或 TopoDS_Face 所对应的 solid 的 id，因为这个 id 需要在全部 converte 好之后再和 old_BrCAD 比较得出，因此无法 calculate hash。整个流程到此就中断了。
...
但是这也意味着我们传递参数的时候不仅要传递 edge id，还要传递 solid id 来标识到底是哪条 edge。
